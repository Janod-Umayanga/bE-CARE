<?php
    ...
    class M_Order_Request {
        ...
        public function getAllOrderRequests($patient_id) {
            $this->db->query('SELECT order_request.*, pharmacist.* FROM order_request INNER JOIN pharmacist ON order_request.pharmacist_id = pharmacist.pharmacist_id  AND is_disabled =0 WHERE order_request.patient_id = :patient_id');
            $this->db->bind(':patient_id', $patient_id);
        ...
        public function getAllAcceptedOrders($patient_id) {
            $this->db->query('SELECT accept_order.*, pharmacist.* FROM accept_order INNER JOIN pharmacist ON accept_order.pharmacist_id = pharmacist.pharmacist_id AND paid=0 AND is_send=0 WHERE accept_order.patient_id = :patient_id');
            $this->db->bind(':patient_id', $patient_id);
